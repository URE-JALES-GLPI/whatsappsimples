...
final class WebhookController extends AbstractController
{
    ...
    public function __invoke(Request $request): Response
    {
        ...
        error_log("[WhatsAppSimples Webhook] Recebido evento: {$event}");

        // 2. Aceita tanto 'messages.upsert' quanto 'messages_upsert'
        ...
        // Se a mensagem foi enviada por nós mesmos (fromMe = true) ignoramos
        if (!empty($key['fromMe'])) {
        ...
        error_log("[WhatsAppSimples Webhook] Gravando mensagem do numero {$phoneNumber}: {$text}");

        // 5. Verifica se já existe um Chat ativo ou anterior para este número
        ...
        // 6. Grava a mensagem recebida vinculada ao chatId
        if ($chatId > 0) {
